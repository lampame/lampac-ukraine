...
namespace LME.UAKino
{
    public class UAKinoInvoke
    {
        ...
        public async Task<List<SearchResult>> Search(string title, string original_title, int year, string imdb_id, CancellationToken ct = default)
        {
            ...
            return await SingleFlightCache.GetOrCreateAsync<List<SearchResult>>(memKey, _hybridCache, async token =>
            {
                ...
                try
                {
                    ...
                    // hydra-aware: RCH (headless-браузер) або проксі підхоплюються автоматично
                    string json = _httpHydra != null
                        ? await _httpHydra.Post(url, body, newheaders: headers)
                        : await Http.Post(_init.cors(url), body, headers: headers, proxy: _proxyManager.Get());

                    if (string.IsNullOrEmpty(json))
                    {
                        _onLog?.Invoke($"UAKino search error: порожня відповідь, url={_init.cors(url)}, useproxy={_init.useproxy} (пошук '{query}')");
                        return null;
                    }
        ...
        public async Task<List<VoiceGroup>> GetPlaylist(string newsId, CancellationToken ct = default)
        {
            ...
            return await SingleFlightCache.GetOrCreateAsync<List<VoiceGroup>>(memKey, _hybridCache, async token =>
            {
